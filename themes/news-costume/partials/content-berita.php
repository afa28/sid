...
<?php if(is_file(LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar'])) { ?>
	<img width="100%" class="img-responsive" src="<?= base_url().LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar']; ?>" alt="<?= $content['judul']; ?>">
<?php } ?>

<div class="panel panel-default">

	<div class="panel-body">
		<div class="single-title"><?= $page_header; ?></div>
		<div class="meta-categories">
			<?php if ( trim($content['kategori'] ) != ''): ?>
				<a href="<?= get_kategori_url( $content['id_kategori'] ); ?>" class="cat-label"><i class="fa fa-tag"></i> <?= ucwords( $content['kategori'] ); ?></a>
			<?php endif; ?>
		</div>
		<div class="meta-content text-right">
			<small>Diterbitkan oleh <?= $content['owner']; ?>, <?= tgl_indo2( $content['tgl_upload'] ); ?></small>

		</div>
		<hr/>
		<!-- Post Content -->
		<div class="artikel-single">
			<?= htmlspecialchars_decode( $content['isi'] ); ?>

			<?php if($content['gambar1']!='' and is_file(LOKASI_FOTO_ARTIKEL."sedang_".$content['gambar1'])): ?>
				<div class="gambar-sampul2"><a class="img-responsive" href="<?= base_url().LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar1']; ?>" title=""><img src="<?= base_url().LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar1']; ?>" class="img-responsive"/></a>
				</div>
			<?php endif; ?>
			<?php if($content['gambar2']!='' and is_file(LOKASI_FOTO_ARTIKEL."sedang_".$content['gambar2'])): ?>
				<div class="gambar-sampul2"><a class="img-responsive" href="<?= base_url().LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar2']; ?>" title=""><img src="<?= base_url().LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar2']; ?>" class="img-responsive"/></a>
				</div>
			<?php endif; ?>
			<?php if($content['gambar3']!='' and is_file(LOKASI_FOTO_ARTIKEL."sedang_".$content['gambar3'])): ?>
				<div class="gambar-sampul2"><a class="img-responsive" href="<?= base_url().LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar3']; ?>" title=""><img src="<?= base_url().LOKASI_FOTO_ARTIKEL.'sedang_'.$content['gambar2']; ?>" class="img-responsive"/></a>
				</div>
			<?php endif; ?>
